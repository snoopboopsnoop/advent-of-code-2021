...
int algorithm(map<string, vector<string>> adjlist, string node, vector<string>& visited, string path, bool doubleUp);
...
int main() {
    ...
    vector<string> visited;
    string path = "start";
    cout << algorithm(adjlist, "start", visited , path, false) << endl;
    ...
    for(string next : adjlist[node]) {
        ...
        if(next == "start") {
            ...
            if(tempDouble == false) {
                ...
                count += algorithm(adjlist, next, temp, currPath, tempDouble);
                continue;
            }
            else continue;
        }
